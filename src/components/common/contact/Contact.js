...
const Contact = () => {
  ...
  //   // Sử dụng API geolocation để lấy vị trí hiện tại của người dùng
  ...
  //     // Xử lý logic tìm kiếm cửa hàng gần nhất tại đây
  //     const nearby = data.filter(async (store) => {
  //       // Kiểm tra xem cửa hàng có địa chỉ không

  //       try {
  //         // Chuyển đổi địa chỉ thành tọa độ sử dụng Geocoding API
  //         getCoords(store.address);

  //         console.log("coordinates>>>>", coordinates);

  //         // Sử dụng công thức tính khoảng cách giữa 2 điểm trên bản đồ (haversine formula)
  //         const R = 6371; // Bán kính Trái Đất (đơn vị km)
  //         const dLat = deg2rad(coordinates.lat - userLocation.lat);
  //         const dLng = deg2rad(coordinates.lng - userLocation.lng);
  //         const a =
  //           Math.sin(dLat / 2) * Math.sin(dLat / 2) +
  //           Math.cos(deg2rad(userLocation.lat)) *
  //             Math.cos(deg2rad(coordinates.lat)) *
  //             Math.sin(dLng / 2) *
  //             Math.sin(dLng / 2);
  //         const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
  //         const distance = R * c;

  //         // Chỉ lấy những cửa hàng có khoảng cách dưới 5km
  //         return distance <= 1;
  //       } catch (error) {
  //         console.error(error);
  //         return false;
  ...
  // // Hàm chuyển đổi địa chỉ thành tọa độ sử dụng Geocoding API
