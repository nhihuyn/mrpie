...
const ShoppingCartSummary: FunctionComponent<Props> = ({
  products,
  onRemoveProduct,
  updateTotalPrice,
}) => {
  ...
  useEffect(() => {
    ...
    if (products.length > 0) {
      totalPrice = products.reduce(
        (total, product) =>
          total + product.price * (counts[product.id] || product.quantity),
        0
      );
    }
          ...
          >
            <div className="product-info flex items-center">
              <img
                src={food}
                alt={product.name}
                className=" w-2/4 h-2/4 md:w-1/3 md:h-1/3 mr-2 "
              />
              <div>
            ...
            <div className="flex justify-center mb-2 rounded-3xl">
              <button
                ...
                className="p-2 bg-red-400 hover:bg-red-200 rounded-l-full text-white block w-8"
              >
                -
              </button>
              ...
              <input
                type="text"
                className="w-10 bg-red-400 border-2 border-red-800 text-center text-white"
              ...
              <button
                ...
                className="p-2 bg-red-400 hover:bg-red-200 rounded-r-full text-white block w-8"
              >
                +
              </button>
            </div>

            <p className=" text right md:text-center mb-2 mobile-hidden">
              {product.price.toLocaleString()} vnd
            </p>
            <p className="text-center mb-2 mobile-hidden">
              {(product.price * (counts[product.id] || 1)).toLocaleString()} vnd
            </p>
            ...
            <div className="text-right col-span-2 md:text-center md:col-span-1 mr-4">
              ...
              <p className=" desktop-hidden">
                {t("Price")}: {product.price.toLocaleString()} vnd
              </p>
              <p className=" desktop-hidden">
                {t("TotalCart")}:{" "}
                {(product.price * (counts[product.id] || 1)).toLocaleString()}{" "}
                vnd
              </p>
            </div>
          </div>
